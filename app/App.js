  ...
  const renderRates = () => (
    <View style={{ flex: 1, padding: 20 }}>
      ...
      <View style={{ marginBottom: 15, zIndex: 2000 }}>
        <TouchableOpacity
          style={{ borderBottomWidth: 1, borderColor: '#CCC', paddingBottom: 5, flexDirection: 'row', justifyContent: 'space-between' }}
          onPress={() => setIsAgentDropdownOpen(!isAgentDropdownOpen)}
        >
          <Text style={{ fontSize: 16, color: selectedSubUser ? '#333' : '#999' }}>
        ...
        {isAgentDropdownOpen && (
          <View style={{ position: 'absolute', top: 30, left: 0, right: 0, backgroundColor: '#FFF', elevation: 5, borderWidth: 1, borderColor: '#CCC', borderRadius: 4 }}>
            {subUsers.map(u => (
              <TouchableOpacity key={u.id} style={{ padding: 15 }} onPress={() => { setSelectedSubUser(u); setIsAgentDropdownOpen(false); }}>
